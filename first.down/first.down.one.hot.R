#create one hots for reading into keras/tensorflow for deeplearning
#also normalizes numeric features with max normalization 

training <- read.csv('first.down.train.csv', header = TRUE)
testing <- read.csv('first.down.test.csv', header = TRUE)
...
training.one.hot.out <- cbind(training.one.hot[, c(-1, -2, -6, -10, -11, -15, -16)], quarter, down, sideoffield, posteam, defteam, playtype, First_Down = training$FirstDown)
test.one.hot.out <- cbind(test.one.hot[, c(-1, -2, -6, -10, -11, -15, -16)], tquarter, tdown, tsideoffield, tposteam, tdefteam, tplaytype, First_Down = testing$FirstDown)
...
write.csv(training.one.hot.out, "first.down.train.one.hot.csv", row.names = FALSE)
write.csv(test.one.hot.out, "first.down.test.one.hot.csv", row.names = FALSE)