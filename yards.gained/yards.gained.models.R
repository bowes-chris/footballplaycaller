...
do.e071.DT <- function(training){
  ...
  tune.control(sampling = c("cross"), cross = 1)
  dec_tree <- tune.rpart(Yards.Gained ~ . , data = training , minsplit = c(1,2,3),
...
{  print("random forest")
  ...
  rf.Fit <- train(Yards.Gained ~ .,data = training,method = "rf",metric = "RMSE",preProc = c("center", "scale"),
...
do.RadialKernelSVM <- function(training)
{
  ...
  grid <- expand.grid(sigma = sigma , C = 2^seq(from=-4,by = 1, to =4))
  print("Radial Kernel SVM")
  ctrl.cross <- trainControl(method = "cv", number = 5,classProbs = TRUE,savePredictions=TRUE)
  svm.Fit <- train(Yards.Gained ~ ., data= training,perProc = c("center", "scale"),
...
{
  set.seed(1)
  grid <- expand.grid(scale = 1, degree = c(1,2,3), C = 2^seq(from=-3,by = 3, to =1))
...
do.KNN <- function(training){
  ...
  grid <- expand.grid(kmax = seq(from=20,to=50,by = 5),distance = c(9,27,81),kernel = "optimal")
  set.seed(313)
  ctrl.cross <- trainControl(method = "cv", number = 10,allowParallel=TRUE)
  knn_fit <- train(Yards.Gained ~ ., data = training,method = "kknn",metric = "RMSE",perProc = c("center", "scale"),tuneGrid = grid,
                   trControl = ctrl.cross)
  knn_fit
}
...
do.CARET.Predict<-function(tune.model,test_data){
  ...
  cm<- confusionMatrix(Pred,test_data$Yards.Gained)
  cm
}
