setwd("/Users/johnrivas/Desktop/UHD/SCHOOL/Fall_2017/Predictive/Project/yards.gained")
nfl <- read.csv("nfl.final.csv")
nfl <- nfl[,-1]

# Only columns that will be used in play type classification models
nfl.pt <- nfl[,c(4:8,10:15, 18:19 ,26,29)]
...
set.seed(123)
intrain <- createDataPartition(y = nfl.pt$Yards.Gained, p = 0.7, list = FALSE) 
assign("training", nfl.pt[intrain,] , envir = .GlobalEnv)
assign("testing",  nfl.pt[-intrain,] , envir = .GlobalEnv)

write.csv(training,"yards.gained.train.csv")
write.csv(testing,"yards.gained.test.csv")
