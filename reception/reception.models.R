...
do.e071.DT <- function(training){
  ...
  dec_tree <- tune.rpart(Reception ~ . , data = training , minsplit = c(1,2,3),
...
{  print("random forest")
  ...
  rf.Fit <- train(Reception ~ .,data = training,method = "rf",metric = "Accuracy",preProc = c("center", "scale"),
...
do.RadialKernelSVM <- function(training)
{
  ...
  svm.Fit <- train(Reception ~ ., data= training,perProc = c("center", "scale"),
...
do.KNN <- function(training){
  ...
  grid <- expand.grid(kmax = seq(from=1,to=5,by = 1),distance = c(1,2,3),kernel = "optimal")
  set.seed(313)
  ctrl.cross <- trainControl(method = "cv", number = 10,allowParallel=TRUE)
  knn_fit <- train(Reception ~ ., data = training,method = "kknn",metric = "Accuracy",perProc = c("center", "scale"),tuneGrid = grid,
                   trControl = ctrl.cross)
  knn_fit
}
...
do.CARET.Predict<-function(tune.model,test_data){
  ...
  cm<- confusionMatrix(Pred,test_data$Reception)
  cm
}
